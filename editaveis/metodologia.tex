...
O \textit{Design Thinking} é uma técnica utilizada para a solução de problemas de forma criativa e iterativa, por meio de etapas cíclicas, com o objetivo de atender às necessidades do ponto central: o usuário. A metodologia adotada para o levantamento de requisitos seguiu os princípios do \textit{Design Thinking}, avançando até a fase de prototipação, enquanto a fase de desenvolvimento e de testes com clientes, será realizada no Trabalho de Conclusão 2 (TCC2).
    ...
    \subsection{Fase de Prototipação}
        ...
        \section*{UC03: Gerenciar Cursos inscritos}
...
\subsubsection{Justificativa da escolha da tecnologia}
No contexto deste trabalho, o NextJS foi escolhido para desenvolver a interface da aplicação, garantindo carregamento eficiente e uma experiência fluida para os usuários. Além disso, o NextJS traz recursos como otimização automática de imagens, roteamento intuitivo e suporte integrado a APIs, facilitando a implementação de uma interface interativa e responsiva para o projeto \cite{nextjs2025}. 
...
Complementando o modelo relacional, a parte descentralizada da arquitetura, composta pela rede IPFS integrada ao OrbitDB, será responsável por armazenar os conteúdos educacionais relacionados às aulas, tais como textos, vídeos, anexos e metadados. Esses dados serão persistidos em documentos no formato JSON, permitindo flexibilidade na representação, característica importante em ambientes distribuídos.

Cada conteúdo de aula será representado como um objeto JSON com uma estrutura definida, que padroniza os principais campos esperados para garantir interoperabilidade e facilitar o consumo pelos sistemas cliente. As tabelas \ref{tab:json-schema-ipfs} e \ref{tab:json-schema-video} apresentam o esquema lógico adotado para esses dados no IPFS.
