...
\subsection{Critérios de aceitação dos requisitos}

Para garantir que os requisitos definidos para o chatbot sejam devidamente compreendidos, implementados e validados, estabelece-se neste trabalho um conjunto de critérios de aceitação. Esses critérios representam condições objetivas que permitem verificar se uma funcionalidade foi implementada corretamente e atende às expectativas previamente estabelecidas.

Segundo Sommerville (2011), os critérios de aceitação são fundamentais para validar requisitos funcionais e não funcionais, atuando como ponto de verificação entre o que foi solicitado e o que foi entregue. No contexto do desenvolvimento ágil e iterativo adotado neste projeto, esses critérios também auxiliam na organização de tarefas e na construção de testes direcionados à verificação da solução.

A formulação dos critérios foi orientada pelos seguintes princípios:
\begin{itemize}
    \item Clareza e objetividade nas condições de sucesso;
    \item Alinhamento direto com os requisitos levantados;
    \item Foco na experiência do usuário final;
    \item Facilidade de mensuração e validação prática.
\end{itemize}

A Tabela~\ref{tab:criterios_aceitacao} apresenta os critérios de aceitação definidos para alguns dos requisitos funcionais mais relevantes do projeto.

\begin{table}[H]
\centering
\caption{Critérios de aceitação de requisitos selecionados}
\label{tab:criterios_aceitacao}
\begin{tabular}{|p{5cm}|p{9cm}|}
\hline
\textbf{Requisito} & \textbf{Critério de Aceitação} \\ \hline
Responder dúvidas frequentes & O chatbot deve retornar uma resposta coerente e correta para ao menos 90\% das perguntas listadas no banco de dados de perguntas frequentes. \\ \hline
Fornecer links diretos para documentos oficiais & Ao ser questionado sobre determinado procedimento, o chatbot deve fornecer um link funcional que leve diretamente ao documento relevante, hospedado em domínio oficial da UnB ou FCTE. \\ \hline
Apresentar prazos importantes & O chatbot deve ser capaz de listar, quando solicitado, os principais prazos do calendário acadêmico vigente, com as respectivas datas e descrições. \\ \hline
Encaminhar para atendimento humano & Quando não for capaz de resolver a dúvida do usuário, o chatbot deve informar claramente a limitação e apresentar um canal de contato (e-mail ou link para atendimento via secretaria ou professor). \\ \hline
Disponibilizar atendimento em múltiplos canais & O chatbot deve ser acessível por pelo menos duas plataformas distintas (por exemplo, WhatsApp e Telegram), com comportamento equivalente em ambas. \\ \hline
\end{tabular}
\end{table}
...
% As imagens apresentadas nas próximas seções demonstram as principais telas e interações do chatbot, permitindo uma análise visual e funcional da solução proposta, bem como auxiliando na validação junto aos usuários e stakeholders.
